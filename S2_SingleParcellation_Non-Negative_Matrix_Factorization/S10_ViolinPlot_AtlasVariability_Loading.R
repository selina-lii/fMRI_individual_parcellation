...
config=readMat('~/DATA/workdir/code/S2_SingleParcellation/config.mat');

Workdir = config$Workdir;
ProjectFolder = config$ProjectFolder;
AnalysisFolder = config$AnalysisFolder;
VisualizationFolder = config$VisualizationFolder;
Variability_Visualize_Folder = paste0(AnalysisFolder, '/Variability_Visualize');
Variability_Mat = readMat(paste0(Variability_Visualize_Folder, '/VariabilityLoading_Median_17SystemMean.mat'));

AtlasLabel_Mat = readMat(paste0(AnalysisFolder, '/Group_AtlasLabel.mat'));
...
for (i in c(1:59412))
{
  ...
  AllData$Label[i] = AtlasLabel_Mat$gp.AtlasLabel.NoMedialWall[i];
}
...
writeMat(paste0(ProjectFolder, '/Corr_NetworkVariability_EffectSize/NetworkVariability.mat'), MedianValue = MedianValue);
...
ggsave('~/DATA/workdir/project/SingleParcellation/SingleAtlas_Analysis/Variability_Visualize/Variability_Loading_Mean_Violin.tiff', width = 20, height = 15, dpi = 600, units = "cm");
